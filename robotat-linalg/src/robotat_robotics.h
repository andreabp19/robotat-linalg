...
/**
 *  MORE FUNCTIONS THAT I COULD ADD BASED ON ROBOTICS TOOLBOX DOCUMENTATION
 *  - rob_oa2r? = convert orientation and approach vectors to rotation matrix
 *  - rob_oa2tr? = convert orientation and approach vectors to homogeneous transformation
 *  - rob_rt2tr? = converts rotation and translation to homogeneous transform
 *  - rob_tr2rt? = viceversa of the above: converts homogeneous transformation to rotation and traslation.
 *  - rob_rpy2tr = roll-pitch-yaw angles to homogeneous transform
 *  - rob_skew = create skew-symmetric matrix
 *  - rob_skewa = create augmented skew_symmetric matrix
 *  - rob_tr2eul = convert rotation matrix to euler angles
 *  - rob_tr2jac = homogeneous transformation matrix (4x4) to (6x6) jacobian matrix
 *  - rob_tr2rpy = converts T or R to roll-pitch-yaw angles (1x3 vector) 
 *  Operaciones de cuaterniones:
 *  - rob_quat_isequal = check if two quaternions are equal
 *  - rob_quat_log = logarithm of quaternion
 *  - rob_quat_matrix = matrix representation of a quaternion
 *  - rob_quat_add = add two quaternions (element-wise)
 *  - rob_quat_sub = subtract two quaternions (element-wise difference of two quaternions)
 *  - rob_quat_exp = exponential of quaternion
 *  - rob_quat_inner = quaternion inner product
 *  - rob_quat_inv = inverse of quaternion
 *  - rob_quat_pow = quaternion raised to the integer power N
 *  - rob_quat_div = quaternion quotient (either with other quaternion or with a scalar)
 *  - rob_quat_mul = multiplication: quaternion*quaternion, quaternion*scalar, scalar*quaternion.
 *  - rob_quat_norm = norm of a quaternion
 *  - rob_quat_prod = product of the elements of a quaternions
 *  - rob_quat_pure = construct a pure quaternion
 *  - rob_quat_sets = set scalar component
 *  - rob_quat_setv = set vector component
 *  - rob_quat_unit = unitize a quaternion
 *  - rob_tr_chain = compound homogeneous transformation matrices from string
 */
